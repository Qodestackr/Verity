...
export const admin = <O extends AdminOptions>(options?: O) => {
  ...
  const permissionsToString = (permissions: string[]): string =>
    permissions.join(",");
            ...
            user: {
              create: {
                async before() {
                  ...
                  let defaultRole = await adapter.findRoleByName(
    ...
    endpoints: {
      createRole: createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          const permissions = ctx.body.permissions.includes(
        ...
        async (ctx) => {
          ...
          if (ctx.query?.filterValue) {
            if (ctx.query.filterField === "role") {
              const role = await adapter.findRoleByName(
                String(ctx.query.filterValue)
              );
              if (!role) {
                throw new APIError("BAD_REQUEST", {
                  message: ERROR_CODES.ROLE_NOT_FOUND,
