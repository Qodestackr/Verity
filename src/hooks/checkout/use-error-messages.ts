...
export const errorMessages = {
  ...
  passwordTooShort:
    "Provided password is too short. Minimum length is 8 characters.",
  passwordTooSimilar:
    "Provided password is too similar to your previous password.",
  passwordTooCommon:
    "Provided password is too common. Use something more fancy.",
...
export const useErrorMessages = <TKey extends string = ErrorCode>(
  customMessages?: Record<TKey, string>
) => {
  ...
  const getMessageByErrorCode = useCallback(
    (errorCode: string) => {
      const formattedMessage =
        messagesToUse[errorCode as keyof typeof messagesToUse];
      if (!formattedMessage) {
